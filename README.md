...
When Phase 16 (Combinatorial Syntax) succeeds, these agents will have developed a true proto-language mapping "Nouns" to objects and "Verbs" to actions to survive an open-ended multi-agent environment. 
By Phase 17 and 18, the goal is to build a translation autoencoder to map their grounded latent representations back to English, allowing us to converse with a non-human AGI whose language maps directly to causal reality, rather than just statistical text patterns.
