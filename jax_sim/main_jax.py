...
def make_sim_step(
    ...
    # detected) and attempts ~4x *below* the real chance baseline (z~-7) --
    # too sparse an encounter rate to bootstrap from, not merely slow.
    # hearth_radius>0 widens each hearth to a (2r+1)^2 region; same fixed
    # centers, same curriculum, same reward math.
    _hearth_radius = int(config.get("hearth_radius", 0))
    ...
    def sim_step(carry, scan_input):
        ...
        # Gate 0 instrument, replaced 2026-09-22 (Cam): deposit count is the
        # outcome of a long behavioral chain observed maybe twice a rollout
        # -- n too small to tell "beginning to learn" from "random" before
        # burning 50 updates finding out. Mean distance from every living
        # agent to its nearest hearth is generated by every agent every
        # step (n ~ population*512 per rollout, not ~2): falling means
        # they're learning to approach; flat at the random-walk expectation
        # means they aren't, and no amount of waiting fixes that.
        _hd_dx = jnp.abs(b_pop.positions[:, None, 0] - grid.hearth_positions[None, :, 0])
        _hd_dy = jnp.abs(b_pop.positions[:, None, 1] - grid.hearth_positions[None, :, 1])
...
def _run_simulation_impl(
    ...
    for ui in range(start_update, n_updates):
        ...
        if step_val % 512 == 0 or T >= 512:
            ...
            # Gate 0 instrument, replaced 2026-09-22 (Cam): deposit count is
            # n~2/rollout, too sparse to tell "beginning to learn" from
            # "random" before burning 50 updates. Mean distance-to-nearest-
            # hearth is generated by every living agent every step (n on the
            # order of population*512 per rollout) -- falling means they're
            # learning to approach; flat at the random-walk expectation
            # means they aren't.
            _hearth_dist_str = "n/a"
            if "hearth_dist_nearest" in rollout_data["blue"]:
                _hd_arr = np.asarray(rollout_data["blue"]["hearth_dist_nearest"])   # (T, N)
                _hd_alive = np.asarray(rollout_data["blue"]["alive"]).astype(bool)  # (T, N)
                _hd_alive_vals = _hd_arr[_hd_alive]
                _hd_mean = float(_hd_alive_vals.mean()) if _hd_alive_vals.size else float("nan")
                # Exact random-walk expectation for the live geometry: mean
                # Chebyshev distance from every cell to its nearest hearth,
                # a fixed function of grid_size and hearth_positions alone
                # (no sampling), computed once per update -- cheap (grid_size^2).
                _gy, _gx = np.meshgrid(np.arange(gs), np.arange(gs), indexing="ij")
                _hp = np.asarray(grid.hearth_positions)
                _rw_dx = np.abs(_gx[..., None] - _hp[None, None, :, 0])
                _rw_dy = np.abs(_gy[..., None] - _hp[None, None, :, 1])
                _rw_dx = np.minimum(_rw_dx, gs - _rw_dx)
                _rw_dy = np.minimum(_rw_dy, gs - _rw_dy)
                _rw_expect = float(np.maximum(_rw_dx, _rw_dy).min(axis=-1).mean())
                _hearth_dist_str = f"{_hd_mean:.2f} (random-walk expectation: {_rw_expect:.2f})"
            ...
            print(
                ...
                f"mean_dist_to_hearth={_hearth_dist_str} | "
